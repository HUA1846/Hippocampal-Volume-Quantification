...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    header_font = ImageFont.truetype("assets/Roboto-Regular.ttf", size=50)
    main_font = ImageFont.truetype("assets/Roboto-Regular.ttf", size=30)

    slice_nums = [orig_vol.shape[2]//3, orig_vol.shape[2]//2, orig_vol.shape[2]*3//4] # is there a better choice?
#     orig_slice = orig_vol[slice_nums[0],:,:]
...
#     nd_img = np.flip((orig_slice/np.max(orig_vol))*0xff).T.astype(np.uint8)
#     y = np.flip((pred_slice/np.max(pred_vol))*0xff).T.astype(np.uint8)
    # This is how you create a PIL image from numpy array
    pil_i = Image.fromarray(orig, mode="L").convert("RGBA").resize((300,300))
    pil_y = Image.fromarray(pred, mode="L").convert("RGBA").resize((300,300))
    # Paste the PIL image into our main report image object (pimg)
    pimg.paste(pil_i, box=(100, 500))
    pimg.paste(pil_y, box=(500, 500))
    ...
    draw.text((0, 0), "HippoVolume.AI", (255, 255, 255), font=header_font)
    draw.multiline_text((10, 50), 
...
#     study_dir = sorted(subdirs, key=lambda dir: os.stat(dir).st_mtime, reverse=True)[0]
    ...
    report_save_path = r"/home/workspace/dicoms/report.dcm"
